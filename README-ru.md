...
> Примечание: в этой версии веб-панель и статические файлы удалены. Управление выполняется через REST API, Telegram-бота или CLI.
...
- Веб-панель в этой сборке отсутствует. Управление выполняется через CLI или встроенного Telegram-бота.

Далее, вы можете создать sudo-администратора для доступа по CLI или API, выполнив команду: 

```bash
marzban cli admin create --sudo
```

Готово! Теперь вы можете использовать эти учетные данные в CLI или любом API-клиенте.
...
По умолчанию API доступен по адресу `http://localhost:8000/api/`. Настроить параметры можно, изменив переменные окружения `UVICORN_HOST` и `UVICORN_PORT`.
</details>
